...
st.title("RTD_SIM — Phase 4 Dashboard (with Debug Panel)")

# Sidebar form
with st.sidebar.form("scenario_form", clear_on_submit=False):
    st.markdown("### Scenario Setup")
    steps = st.number_input("Steps", min_value=50, max_value=2000, value=300, step=50)
    agents_n = st.number_input("Agents", min_value=1, max_value=200, value=50, step=1)
    ...
    smooth_routes = st.checkbox("Smooth routes (densify)", value=False)
...
if df is not None and not df.empty:
    ...
    st.dataframe(df.head(5), width=1000)
    sample_locations = df["location"].dropna().head(5).tolist() if "location" in df.columns else []
    sample_routes = df["route"].dropna().head(3).tolist() if "route" in df.columns else []
    st.write("Sample locations:", sample_locations)
    st.write("Sample routes:", sample_routes)
else:
    st.info("No data available for debug.")
