...
UK_GTFS_OPERATORS = {
    ...
    "Stagecoach West Scotland": "f-gcpv-stagecoachbus",
}
...
def download_gtfs_feed(
    ...
    """Download a GTFS feed from TransitLand by Feed ID or direct URL."""
    import urllib.request
    import shutil
    from pathlib import Path
    ...
    if operator_id_or_url.startswith('http'):
        ...
        # Convert Operator ID (o-) to Feed ID (f-) just in case legacy IDs are passed
        feed_id = operator_id_or_url
        if feed_id.startswith('o-'):
            feed_id = 'f-' + feed_id[2:]
            
        # Direct download endpoint bypasses metadata lookup failures
        download_url = f"{TRANSITLAND_API_BASE}/feeds/{feed_id}/download"
        if api_key:
