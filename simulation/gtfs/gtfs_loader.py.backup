...
class GTFSLoader:
    ...
    def __init__(
        ...
        """
        Args:
            feed_path:      Path to .zip file or directory containing GTFS CSVs
            fuel_overrides: {route_id: 'electric'|'diesel'|'hydrogen'|'hybrid'}
                            Takes precedence over all inference.
            service_date:   'YYYYMMDD' string — filter to services active on this
                            date.  None = load all services regardless of date.
        """
    ...
    def load(self) -> 'GTFSLoader':
        """Parse all available GTFS files. Returns self for chaining."""
        if self._loaded:
            return self
        ...
        self._loaded = True
        logger.info(
            "GTFS loaded: %d stops, %d routes, %d trips, %d shapes",
            len(self.stops), len(self.routes),
            len(self.trips), len(self.shapes),
        )
        return self

    def _read_csv(self, name: str) -> Optional[List[Dict[str, str]]]:
        """Read a GTFS CSV file and return list of row dicts, or None if missing."""
        if self.feed_path.suffix.lower() == '.zip':
            try:
                with zipfile.ZipFile(self.feed_path) as zf:
                    names = zf.namelist()
                    # Handle feeds where the CSV is in a subdirectory
                    match = next(
                        (n for n in names if n.endswith(f'/{name}') or n == name),
                        None,
                    )
                    if match is None:
                        return None
                    with zf.open(match) as f:
                        text = f.read().decode('utf-8-sig')  # strip BOM
            except KeyError:
                return None
        else:
    ...
    def _parse_calendar(self) -> None:
        rows = self._read_csv('calendar.txt')
        if not rows:
            return

        if self.service_date is None:
            return  # no date filter — all services active
        ...
        try:
            ...
            logger.warning("calendar date parse failed: %s — loading all services", exc)
    ...
    def _parse_calendar_dates(self) -> None:
        rows = self._read_csv('calendar_dates.txt')
        if not rows or self.service_date is None or self._active_services is None:
            return
        for row in rows:
    ...
    def _parse_stops(self) -> None:
        rows = self._read_csv('stops.txt')
        if not rows:
            logger.warning("GTFS: stops.txt missing or empty")
            return
        for row in rows:
    ...
    def _infer_fuel_type(self, route_id: str, route_type: int, desc: str, color: str) -> str:
        """Infer fuel type from route attributes."""
        ...
        # Rail modes default to electric (most UK rail is overhead electric)
    ...
    def _parse_routes(self) -> None:
        rows = self._read_csv('routes.txt')
        if not rows:
            logger.warning("GTFS: routes.txt missing or empty")
            return
        for row in rows:
    ...
    def _parse_trips(self) -> None:
        rows = self._read_csv('trips.txt')
        if not rows:
            logger.warning("GTFS: trips.txt missing or empty")
            return
        for row in rows:
    ...
    def _parse_stop_times(self) -> None:
        rows = self._read_csv('stop_times.txt')
        if not rows:
            logger.warning("GTFS: stop_times.txt missing or empty")
            return

        active_trips = set(self.trips.keys())
        count = 0
        ...
        for row in rows:
            ...
            dep_s = _parse_time_s(dep_str)
            arr_s = _parse_time_s(arr_str)
            if dep_s < 0:
                continue
            try:
        ...
        logger.info("GTFS: parsed %d stop_time records across %d trips", count, len(self.stop_times))

    def _parse_shapes(self) -> None:
        rows = self._read_csv('shapes.txt')
        if not rows:
            logger.debug("GTFS: shapes.txt absent — routes will render without shape geometry")
            return
        ...
        logger.info("GTFS: parsed %d shapes", len(self.shapes))
