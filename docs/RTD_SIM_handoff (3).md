...
| 9 | ✅ Complete | Story ingestion service, OLMo 2 integration — tested 2026-03-21, viable ≤10 agents |
...
### OLMo Phase 9 Testing + BDI Mode Filter (this session)
- **OLMo test:** PASS. 6/10 agents used LLM extraction (34–57s warm). 4/10 timed out and fell back to rule_based correctly. Plan quality: contextually appropriate (eco_warrior peak-hour → minimize_time, shopping_trip → minimize_carbon).
- **BDI mode filter bug:** personal agents (`vehicle_type=personal`, `vehicle_required=False`) were offered `hgv_electric`, `truck_electric`, `van_electric` and choosing them due to cost function. Fixed as Step 3.6 in `bdi_planner._filter_modes_by_context`.
- **LLM backend timeout:** increased from 60s → 180s in `services/llm_client.py` (GIN logs confirmed 500 errors at exactly 120s due to macOS RAM pressure evicting the model).
- **LLM backend sidebar selector:** `rule_based | olmo | claude` radio added to `_render_advanced_features`. Wired through `SimulationConfig.llm_backend` → `create_planner(llm_backend=...)` → `ContextualPlanGenerator(llm_backend=...)`. Default remains `rule_based`.
- **Startup automation:** `services/startup_manager.py` created. Checks Ollama + Redis on first Streamlit load, auto-starts Ollama via `subprocess.Popen(["ollama", "serve"])`, pulls model if not present. Single launch command: `PYTHONPATH=. python3 -m streamlit run ui/streamlit_app.py`.
- **OLMo auto-start:** `LLMClient.ensure_ollama_running()` and `ensure_model_pulled()` class methods added.
- **requirements.txt:** Added `paho-mqtt>=2.0.0`, `websockets>=12.0`, `pyvis>=0.3.2`, `scikit-learn>=1.3.0`, `shap>=0.44.0`. Phase 13 deps documented. ODTP/Eclipse Ditto integration notes added.
...
### OLMo Test Results — COMPLETED 2026-03-21
```
Connectivity test:  PASS
Ollama version:     0.17.7, CPU-only (M3, 36GB unified memory)
Model:              OLMo 2 1124 13B Instruct, Q4_K_Medium (7.78 GiB)
KV cache:           3.2 GB (CPU)

LLM extraction results (10 agents, 50 steps):
  Successful:  6/10 agents  (34–57s per agent, warm)
  Timed out:   4/10 agents  (hit 120s hard timeout → rule_based fallback)
  Fallback:    Works correctly in all timeout cases

Plan quality (successful extractions):
  eco_warrior + morning_commute → objective=minimize_time, fixed=True
    Reasoning: "schedule-fixed due to morning peak period, prioritizes
    minimizing travel time to reduce stress and ensure punctuality"
    ✅ Correctly overrides eco preference under schedule pressure

  eco_warrior + shopping_trip → objective=minimize_carbon, fixed=False
    Reasoning: "trip scheduled due to commitments to minimize carbon
    footprint; regulatory constraints encouraging EVs"
    ✅ Correct — flexible errand, eco preference dominates

  concerned_parent + shopping_trip → objective=minimize_time, fixed=True
    Reasoning: "fixed times for school runs and shopping; reliability
    critical for safety and punctuality of children"
    ✅ Correct — school-run context inferred without being told

Latency profile:
  Cold start (first request):  ~21s (model load)
  Warm (subsequent):           34–57s
  Memory pressure spike:       >120s → timeout (macOS evicts model from RAM)

Bug found: eco_warrior (vehicle_type=personal) was offered hgv_electric,
  truck_electric, van_electric and chose them due to cost function.
  Fixed in bdi_planner.py Step 3.6 — personal agents now filtered to
  {walk, bike, e_scooter, bus, car, ev, cargo_bike} only.

Decision:
  OLMo VIABLE for small research runs (≤10 agents) and Phase 10b agent
  type plan generation. Default remains rule_based for normal simulation.
  Sidebar radio added: rule_based | olmo | claude.
  Timeout increased to 180s in llm_client.py (was 60s, GIN showed 2m0s=120s).
  Auto-start added via services/startup_manager.py.
```
...
## Startup Automation — No Extra Terminals Required

RTD_SIM must be launchable with a single command. `services/startup_manager.py` handles all service dependencies automatically.

**Single launch command:**
```bash
cd /Users/theolim/AppDev/RTD_SIM
PYTHONPATH=. python3 -m streamlit run ui/streamlit_app.py
```

**What StartupManager does on first load (in Streamlit session state):**
1. Pings `http://localhost:11434/api/tags` — if Ollama not running, launches `ollama serve` as a detached subprocess and waits up to 10s
2. If model not pulled, runs `ollama pull olmo2:13b` (shows progress, 30min timeout)
3. Pings Redis on port 6379 — if unavailable, logs warning and sets event bus to in-memory fallback (simulation continues)
4. Renders `🔧 Service Status` expander in sidebar with green/amber/red indicators and install instructions

**Integration into `streamlit_app.py`:**
```python
# Add near top of streamlit_app.py, after st.set_page_config():
from services.startup_manager import get_startup_manager
if "startup_done" not in st.session_state:
    manager = get_startup_manager()       # checks & starts services
    manager.render_status_sidebar()
    st.session_state.startup_done = True
```

**Ollama is only started if `llm_backend='olmo'` is selected** in the sidebar — rule_based runs never touch Ollama. The `ensure_ollama_for_llm()` method is called from `simulation_runner.py` just before agent creation when `config.llm_backend == 'olmo'`.

**Service dependency matrix:**

| Service | Required for | Auto-start | Fallback if absent |
|---------|-------------|------------|-------------------|
| Ollama | `llm_backend=olmo` | ✅ via subprocess | rule_based |
| Redis | `enable_event_bus=True` | ❌ system service | in-memory queue |
| Mosquitto (Phase 13) | MQTT sensor feeds | ❌ not yet | synthetic events |
| Eclipse Ditto (Phase 13) | Digital twin federation | ❌ Docker | local-only mode |

---

## UI, Visualisation and Data Architecture Plan

This section captures the design decisions needed before Phase 10 and 13 code is written. Architecture must be locked before implementation — retrofitting is expensive.

### Current UI state (Streamlit)

RTD_SIM runs as a single-page Streamlit app. The current tab structure:
- Simulation Config (sidebar)
- Live Map (agent routes, pydeck)
- System Dynamics (SD charts)
- Analytics (SHAP, mode share, tipping points)
- Agent Cognition (pyvis social network, belief drift, Markov habits)
- Policy Diagnostics
- Combined Scenarios

**What Streamlit is good for:** rapid research UI, parameter exploration, charts. **What it cannot do:** sub-second live updates (Streamlit re-runs the whole script on each interaction), 3D spatial visualisation at national scale, collaborative multi-user sessions, proper WebSocket push from simulation to UI.

### Target UI architecture (Phase 10 + 13)

```
┌─────────────────────────────────────────────────────────────────┐
│  Streamlit (research/config layer)                              │
│  Parameter config, scenario selection, batch results, SHAP,    │
│  ODTP run management, export                                    │
│  Port 8501                                                      │
└──────────────────────┬──────────────────────────────────────────┘
                       │ HTTP API (FastAPI — already exists Phase 9)
┌──────────────────────▼──────────────────────────────────────────┐
│  RTD_SIM Simulation Core (Python)                               │
│  BDI agents, SD model, social network, policy engine           │
│  Exposes: /run, /status, /snapshot, /stream (SSE)              │
└──────────────────────┬──────────────────────────────────────────┘
                       │ Server-Sent Events / WebSocket
┌──────────────────────▼──────────────────────────────────────────┐
│  Live Visualisation Layer (Deck.gl / Kepler.gl)                 │
│  UK transport network, agent positions, corridor heatmaps,     │
│  ferry routes, policy zone overlays                            │
│  Embedded as iframe in Streamlit OR standalone on port 3000    │
└─────────────────────────────────────────────────────────────────┘
```

**Recommended live viz stack:** Deck.gl (already via pydeck) + H3 hexagonal binning for agent density. For national UK scale, individual agent dots are unreadable at >500 agents — switch to H3 mode share heatmap at zoom < 10, individual agents at zoom > 12.

### Data architecture

**Simulation state schema (design now for Ditto compatibility):**

Each agent needs a stable schema that maps to a Ditto Thing. Define this in `simulation/config/agent_schema.py`:

```python
# Agent Thing Description (maps to Eclipse Ditto Thing model)
AGENT_THING_SCHEMA = {
    "thingId": "rtd-sim:{agent_id}",
    "features": {
        "position":     {"properties": {"lon": float, "lat": float}},
        "mode":         {"properties": {"current": str, "history": list}},
        "beliefs":      {"properties": {"eco": float, "cost": float, "time": float}},
        "desires":      {"properties": {"eco": float, "cost": float, "time": float, "comfort": float}},
        "markov":       {"properties": {"habit_strength": dict, "streak": int}},
        "journey":      {"properties": {"distance_km": float, "emissions_g": float, "arrived": bool}},
    }
}

# Infrastructure Thing Description
CHARGER_THING_SCHEMA = {
    "thingId": "rtd-sim:charger-{station_id}",
    "features": {
        "occupancy":    {"properties": {"ports_used": int, "ports_total": int}},
        "location":     {"properties": {"lon": float, "lat": float}},
        "grid_load_kw": {"properties": {"current": float}},
    }
}
```

**Time series storage:** SD history and mode share time series should be written to Parquet files per run (not just in-memory lists). This enables ODTP S3 upload and Ditto snapshot replay without rerunning the simulation. Add to `simulation_loop.py` output:
```python
# At end of run, before returning loop_results:
import pandas as pd
pd.DataFrame(system_dynamics_history).to_parquet(
    f"logs/sd_history_{run_id}.parquet"
)
```

**OSM graph caching:** Currently uses `GraphManager` with pickle cache. For Phase 10 national scale, switch to GeoParquet per tile (smaller, faster partial load, QGIS-compatible for debugging).

### Visualisation roadmap

| Phase | Feature | Tech |
|-------|---------|------|
| Now | Fix agent label overflow in cognition tab | ✅ Done (label=' ') |
| 10a | UK corridor map with H3 density heatmap | pydeck H3Layer |
| 10b | Per-agent-type colour coding (freight orange, passenger blue, ferry teal) | pydeck ScatterplotLayer |
| 13a | Live position streaming (SSE push → pydeck update) | FastAPI SSE + pydeck |
| 13b | 3D UK transport network with elevation | Deck.gl TerrainLayer |
| 13c | OpenTwins 3D embed as iframe | OpenTwins WebGL |

### Key architectural decisions to lock before Phase 10

1. **FastAPI as the simulation API layer** — Phase 9's story ingestion service already uses FastAPI. Extend it with `/api/v1/simulation/run`, `/status`, `/stream`. This gives ODTP a clean HTTP interface and avoids Streamlit's rerun model for live updates.

2. **Run ID scheme** — Every simulation run needs a stable UUID that links: Streamlit session → log file → Parquet outputs → Ditto Thing prefix → ODTP execution ID. Generate at `SimulationConfig` creation time: `config.run_id = uuid4().hex[:12]`.

3. **Agent ID stability** — Agent IDs must be deterministic given `(run_id, agent_index, user_story, job_story, seed)` so Ditto Things can be replayed. Currently uses `secrets.randbits(32)` — fine for simulation, breaks Ditto replay. Add `config.deterministic_ids: bool = False` flag.

4. **Corridor definition format** — User-defined corridors (map pins or location inputs) must serialize to a stable format that ODTP can store and replay. Use GeoJSON Feature with bbox property. Existing `_locations_to_bbox()` in `sidebar_config.py` is the right function — just needs to output GeoJSON.
