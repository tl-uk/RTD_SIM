...
| 10 | ❌ Not started | Generalised spatial environment — abstract Edinburgh bbox to runtime config |
| 11–12 | ✅ Complete | Policy diagnostics, combination report, agent combinations |
| 13 | ❌ Not started | MQTT / IoT real-time integration |
...
## Phase 10 — Generalised Spatial Environment (Not Started)

Abstract the Edinburgh bounding box (`bbox`) and OSM graph loading so any city can be specified at runtime without hardcoded coordinates.

**Key files to modify:**
- `simulation/config/simulation_config.py` — add `region_bbox`, `region_name`, `osm_cache_dir` as runtime parameters
- `simulation/setup/environment_setup.py` — replace hardcoded Edinburgh bbox with `config.region_bbox`
- `ui/sidebar_config.py` — expose city/region picker or bbox input

**Goal:** Pass any `(west, south, east, north)` bbox tuple at runtime. Cache OSM graphs by bbox hash. Support named presets (Edinburgh, Glasgow, Belfast, London).

---

## Phase 13 — MQTT / IoT Real-Time Integration (Not Started)

Wire real sensor data (EV charger occupancy, grid load, traffic counts) into the simulation via MQTT broker.

**Planned architecture:**
- MQTT subscriber service listening on `rtd_sim/sensors/#` topics
- Sensor data fed into `StreamingSystemDynamics.update()` as real `infrastructure_capacity` and `grid_load`
- Replace synthetic event generator with real event stream when broker available
